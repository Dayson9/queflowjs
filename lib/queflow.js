...
function stringBetween(str, f, s) {
  const startIndex = str.indexOf(f);
  if (startIndex === -1) return ""; // f not found

  const endIndex = str.indexOf(s, startIndex + f.length); // Search s after f
  if (endIndex === -1) return ""; // s not found

  return str.slice(startIndex + f.length, endIndex);
}
...
function evaluateTemplate(reff, instance, flag) {
  ...
  try {
    out = reff.replace(EVAL_REGEX, (match) => {
      ...
      return flag ? rendered : sanitizeString(rendered);
    });
  } catch (error) {
...
function jsxToHTML(jsx, instance, subId) {
  ...
  const out = evaluateTemplate(div.innerHTML, instance);
  div.remove();
  return [out, data];
}
...
function updateComponent(ckey, obj, _new, flag) {
  let dataQF;
  ...
  for (let d of dataQF) {
    ...
    if (child) {
      if (needsUpdate(template, ckey)) {
        let evaluated = evaluateTemplate(template, obj, flag);
