...
var QueFlow = {
    ...
    List : (obj, template) =>{
    let output = "", sample = "", len = QueFlow.tempLen(template), i = 0;
  
    if(obj instanceof Array){
    
    obj.forEach((entr, index) =>{
    if(typeof entr == "string"){
    for(i = 0; i< len; i++){
    let btw = QueFlow.strBetween(template, "{{", "}}")
    sample = template.replaceAll(btw, btw+"["+index+"]");
    
    }
    output+= sample;
    }else{
    /** 
data = [{src: "jpg", alt: "img"}, {src : "jpg", alt: "img"}]
QueFlow.List(data, "<img src='{{src}}' alt='{{alt}}'>")

data = ["Hello", "world"]

QueFlow.List(data, "<span>{{data}}</span>")

data = {src: "jpg", alt: "img"}
 ...
 **/
    ...
    jsxToHTML: function(jsx, indicator){
        ...
        children.forEach((c) =>{
        if(!this.parentType(c)[0]){
        let attr = this.attr(c);
        attr.push({attribute: "innerText", value: c.innerText});
       
        if(indicator){
        c.dataset.qfid = "qf"+this.counterQF;
        this.counterQF++;
        }else{
        ...
        out = this.parseQF(attr, div.innerHTML, c.dataset.qfid);
        }
        });
         }  catch (error) {
    ...
    iRender: function(selector){
        ...
        renderComponent: function(name, props, selector) {
        let jsx = QueFlow.components[name].jsx;
        let rendered = mustache.render(jsx, props);
        QueFlow.Render(rendered, selector);
        }
};
