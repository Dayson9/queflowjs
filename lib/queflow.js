  ...
  switch (key) {
    ...
    default:
      if (key.indexOf("style.") > -1) {
        ...
        if (child.style[sliced] !== evaluated) child.style[sliced] = evaluated;
      } else {
    ...
    if (typeof data == "object") {
      ...
      if (this.isReactive) {
        const processData = (item) => {
          ...
          const indexedTemplate = addIndexToTemplate(template, this.index, this);
      ...
      console.error(`RenderWith(data, position), 'data' must be an object.\n\nError sourced from Template: '${this.name}'`)
    }
  }
